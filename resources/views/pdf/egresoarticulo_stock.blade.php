...
<html>
<head>
	<title>Detalle Egreso de Productos de Stock</title>
</head>
<style>
    #watermark {
        position: fixed;
...
<body>
    <header>
	   <p>GOBIERNO AUTONOMO DEPARTAMENTAL DEL BENI<br>UNIDAD DE ALMACENES MATERIALES Y SUMINISTROS<br>Artículos/Productos Egresados de Stock - Trinidad, <?php $time = time(); echo date("d-m-Y", $time); ?></p>
    </header>
        ...
        <table cellspacing="0" width="100%" align="center" border="1" style="font-size: 7pt">
            ...
            @foreach($articulos as $art)
                ...
                <tr>
                    ...
                    <td style="width: 50px">{{$art->fechasalida}}</td>
                    <td style="width: 50px">{{$art->codigopedido}}</td>
                        <td style="width: 50px">
                            @foreach($edet->solicitudcompra->preventivo as $prev)
                            {{$prev->numeropreventivo}};
                            @endforeach
                        </td>
                    <td style="width: 250px">{{$art->direccionadministrativa->nombre}}<br><strong>{{$art->unidadadministrativa->nombre}}</strong></td>
    ...
    <div class="row">
        <table width="100%" align="center" style="font-size: 7pt">
            <tr>
                <th style="text-align: right">Total - Detalle de Egreso de Stock: {{NumerosEnLetras::convertir($suma_Total,'Bolivianos',true)}}</th>
            </tr>
        </table>
    </div><br>
